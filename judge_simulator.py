...
# Your API key (recommended: set env var GROQ_API_KEY / LLM_API_KEY)
LLM_API_KEY = __import__("os").getenv("GROQ_API_KEY") or __import__("os").getenv("LLM_API_KEY", "")

# Model to use (leave empty for default, or specify like "gpt-4o", "claude-3-5-sonnet-20241022", etc.)
LLM_MODEL = __import__("os").getenv("GROQ_MODEL", "")  # Optional override (e.g., llama-3.1-70b-versatile)
    ...
    def complete(self, prompt: str, system: str = None) -> str:
        ...
        req = urlrequest.Request(
            ...
            headers={
                ...
                "User-Agent": "magicpin-judge-simulator/1.0 (+https://magicpin.com)",
            }
        )
        try:
            resp = urlrequest.urlopen(req, timeout=TIMEOUT_LLM)
            ...
            if e.code == 403 and "1010" in body:
                raise RuntimeError(
                    "Groq HTTP 403 (Cloudflare 1010 block). "
                    "This is usually IP/network reputation or edge-security blocking, not prompt logic."
                )
